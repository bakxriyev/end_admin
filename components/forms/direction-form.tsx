...
export function DirectionForm({ direction, onSave, onCancel, setToast }: DirectionFormProps) {
  const [formData, setFormData] = useState(direction || {})
  const [loading, setLoading] = useState(false)
  const [photoPreview, setPhotoPreview] = useState(direction?.photo)
  const [videoPreview, setVideoPreview] = useState(direction?.video)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = direction ? `${BACKEND_URL}/direction/${direction.id}` : `${BACKEND_URL}/direction`
      ...
      if (response.ok) {
        setToast({ message: direction ? "Direction updated" : "Direction created", type: "success" })
        onSave()
      } else {
        setToast({ message: "Failed to save direction", type: "error" })
      }
    } catch {
      setToast({ message: "Error saving direction", type: "error" })
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
      <h2 className="text-2xl font-bold mb-4">{direction ? "Edit Direction" : "Add New Direction"}</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex gap-2">
          <Button type="submit" disabled={loading} className="bg-emerald-600 flex-1">
            ...
            {direction ? "Update" : "Create"} Direction
          </Button>
